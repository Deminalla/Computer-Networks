...
for rezultatas in socket.getaddrinfo(hostas, portas, socket.AF_UNSPEC, socket.SOCK_STREAM):
    ...
    try:
        s = socket.socket(adresu_seima, soketo_tipas, protokolas) #sukuriam socket
...
siuntimui = input() 
s.send(bytearray(siuntimui,'utf-8')) #tekstine eilute konvertuojam i baitus and then we send that
gavome = s.recv(1024) #receive
s.close()
print('Gauta ...', str(gavome)) #convert bytes to string
