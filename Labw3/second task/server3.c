...
#define PORT_TO_S2 "20002"
#define PORT_TO_C "20003"
// #define HOST "::1"
...
int main(int argc, char const* argv[])
{
	...
	int data = recv(client_socket, message_from_s2, sizeof(message_from_s2), 0); 
	if (data < 0) {
		perror("Coudln't receive");
		exit(1);
	}
	printf("Message from S2: %s\n", message_from_s2);
	...
	int message_length = strlen(message_from_s2);

    char message_to_c[15] = {0};

    strcpy(message_to_c, message_from_s2);
    char size[2] = {0};
    sprintf(size, "%d", message_length);
    
	strcat(message_to_c, size);

    printf("Sending message to C: %s\n", message_to_c);
	if (send(socket_fd, message_to_c, strlen(message_to_c), 0) < 0 ){
		perror("Can't send\n");
		exit(1);
	}
